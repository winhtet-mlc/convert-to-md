...
class FixLatexPreprocessor(Preprocessor):
    ...
    def preprocess_cell(self, cell, resources, index):
        ...
        if cell.cell_type == "markdown":
            ...
            cell.source = self._fix_latex_escaped_asterisks(cell.source)
        elif cell.cell_type == "code":
            if len(cell.outputs) > 0 and 'data' in cell.outputs[0] and 'text/latex' in cell.outputs[0].data:
                cell.outputs[0].data["text/latex"] = "<div>" + cell.outputs[0].data["text/latex"] + "</div>"
            elif len(cell.outputs) > 0 and 'data' in cell.outputs[0] and 'text/html' in cell.outputs[0].data:
                cell.outputs[0].data["text/html"] = "<div>" + cell.outputs[0].data["text/html"] + "</div>"
    ...
    def _fix_latex_escaped_asterisks(self, text):
        """Replace '_' by '\_' inside LaTeX expressions delimited by
        \[ ... \] or \( ... \)."""
        inline_math = r'\\\((.+?)\\\)'
        display_math = r'\\\[(.+?)\\\]'
        double_escape = lambda m: re.sub(r'(?<!\\)\*', r'\\*', m.group(0))
        new_text = re.sub(inline_math, double_escape, text, flags=re.S)
        new_text = re.sub(display_math, double_escape, new_text, flags=re.S)
        return new_text 
